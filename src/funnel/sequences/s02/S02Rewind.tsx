/**
 * S02_03_REWIND — Microvideo Rewind
 * Psychological/editorial rewind of what Andrés just observed.
 * Pure video: no text overlays, no glitch effects, no VHS filters.
 */
...
export const S02Rewind: React.FC = () => {
  ...
  // Initiate playback of S02 rewind video
  useEffect(() => {
    if (hasStartedRef.current) return;
    hasStartedRef.current = true;

    const startRewind = async () => {
      const ok = await playAsset(VIDEO_ASSETS.S02_REWIND, {
        sequence: "S02_ALGO_SALIO_MAL",
        screen: "S02_03_REWIND",
      });
      if (!ok) {
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      ...
      markVideoCompleted({
        sequence: "S02_ALGO_SALIO_MAL",
        screen: "S02_03_REWIND",
        asset: VIDEO_ASSETS.S02_REWIND,
      });

      // Immediate canonical transition to S02_04_MIRROR
      setCurrentScreen("S02_04_MIRROR");
    };

    const handleError = () => {
      console.warn("[MPN Video S02_03] Media load error event triggered");
    };
    ...
    >
      <div className="relative w-full h-full max-w-[calc(100dvh*9/16)] max-h-[100dvh] aspect-[9/16] bg-transparent flex items-center justify-center">
        ...
        {needsGestureToResume && status !== "playing" && !hasEnded && (
          <div
            id="video-resume-overlay-s02-03"
        ...
        {status === "audio_blocked" && !needsGestureToResume && (
          <div
            id="video-audio-blocked-overlay-s02-03"
          ...
          >
            <div className="max-w-xs flex flex-col items-center gap-4">
              ...
              <button
                id="btn-activate-video-sound-s02-03"
                type="button"
        ...
        {status === "load_error" && (
          <div
            id="video-load-error-overlay-s02-03"
          ...
          >
            <div className="max-w-xs flex flex-col items-center gap-4">
              ...
              <button
                id="btn-retry-video-load-s02-03"
                type="button"
                onClick={retryPlayback}
              ...
              {import.meta.env.DEV && (
                <div className="mt-3 p-3 bg-neutral-900/90 border border-neutral-800 rounded text-left text-[11px] font-mono text-neutral-400 space-y-2">
                  ...
                  <p className="text-neutral-300 break-all">
                    Asset: <span className="text-white">{VIDEO_ASSETS.S02_REWIND}</span>
                  </p>
                  <button
                    id="btn-dev-simulate-ended-s02-03"
                    type="button"
                    onClick={() => {
                      markVideoCompleted({
                        sequence: "S02_ALGO_SALIO_MAL",
